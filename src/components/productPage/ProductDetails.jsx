...
const ProductDetails = ({product}) => {
  ...
  return (
    <div className="lg:w-[60%] flex flex-col justify-center items-center lg:items-start gap-2.5 sm:gap-7 lg:gap-5 px-2.5 lg:px-0">
        ...
        <div className="flex justify-center items-center gap-2.5">
          {
            product.sizes.map((size) => (
              <span className={`rounded border w-[2rem] py-1 text-center font-semibold cursor-pointer hover:scale-105 text-white ${selectSizeOfProduct === size ? `bg-gray-800` : `bg-gray-500`}`} key={size} onClick={() => setSelectSizeOfProduct(size)}>{size}</span>
            ))
          }
        </div>
